...
int main(int argc, char* argv[]) {
	...
	int sockets[2], count=0;
	char buf[1024], word[1024], c;
	
	do{
		c = fgetc(ptr);
		if(c==EOF) {break;}
		word[count] = toupper(c);
		count++;
	}while(c!=EOF);
	...
	if ((pid = fork()) < 0) {
		...
		if (write(sockets[CHANNEL1], word, sizeof(word)) < 0)
...
//basicamente posso escrever e ler de ambos os lados, o pai escreve e le no channel0 e o filho no channel1
